...
function AgentIconSm() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"/></svg>; }
function AgentIconLg() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"/></svg>; }
...
// ── Agent Config ───────────────────────────────────────────────────────
const AGENT_NAME      = 'Ticket Agent';
const AGENT_SHORT     = 'Ticket';
const AGENT_SUBTITLE  = 'Ticket management, triage & resolution tracking';
const AGENT_SYSTEM    = `You are an expert IT ticket management AI agent called "Ticket Agent" for Commedia AI Operations. You help engineers create, triage, escalate, and resolve network tickets. Provide ticket status, priority analysis, SLA tracking, and resolution guidance. Be concise and structured. Use bullet points and **bold** for key terms.`;
const CAPABILITIES    = ["Ticket creation & triage", "SLA & priority tracking", "Auto-escalation rules", "Resolution analytics"];
const PROMPTS         = ["Create a new ticket", "Check open tickets", "Escalate high priority ticket", "Analyze resolution time", "Find tickets by site", "Show SLA breaches"];
const PLACEHOLDER     = 'Describe your ticket or issue...';
const INITIAL_CONTENT = `Hi! I'm your **Ticket Agent**. Ticket management, triage & resolution tracking.\n\nHow can I help you today?`;
...
function Bubble({ msg }: { msg: Message }) {
  ...
  return (
    <Box sx={{ display:'flex', flexDirection:isUser?'row-reverse':'row', alignItems:'flex-end', gap:1.2, mb:2 }}>
      ...
      {/* ── CHANGED: user bubble avatar now uses blue agent theme ── */}
      {isUser  && <Box sx={{ width:30, height:30, borderRadius:'9px', background:`linear-gradient(135deg, ${AGENT_COLOR} 0%, #1d4ed8 100%)`, border:`1px solid ${AGENT_COLOR}60`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, mb:0.2 }}><Typography sx={{ fontSize:'0.7rem', fontWeight:800, color:'#fff' }}>U</Typography></Box>}
      <Box sx={{ maxWidth:'70%' }}>
...
const agentDropdownSx = {
  background: '#0f0f0f', border: `1px solid #2a2a2a`, borderRadius: '10px', mt: 1, minWidth: 170,
  boxShadow: '0 8px 32px rgba(0,0,0,0.7)',
  '& .MuiMenuItem-root': {
    color: '#bbb', fontSize: '0.8rem', py: 1, px: 1.8, gap: 1.2, transition: 'background 0.15s',
    '&:hover': { background: '#1a1a1a', color: '#fff' },
    '&.Mui-selected': { background: '#1e1e1e' },
  },
};

function ChatNavbar({ title, archivedCount, onOpenArchive, currentAgentId = 2 }: { title: string; archivedCount: number; onOpenArchive: () => void; currentAgentId?: number }) {
  ...
  const derivedAgentId =
    Number(
      Object.entries(AGENT_ROUTES).find(([, path]) =>
  ...
  return (
    <Box sx={{ px:3, borderBottom:`1px solid ${BORDER}`, display:'flex', alignItems:'center', justifyContent:'space-between', minHeight:64, flexShrink:0 }}>
      ...
      {/* Right: select agent + archive + user */}
      <Box sx={{ display:'flex', alignItems:'center', gap:1 }}>

        {/* Agent dropdown with label */}
        <Box sx={{ display:'flex', alignItems:'center', gap:0.8 }}>
          <Typography sx={{ fontSize:'0.7rem', color:'#555', fontWeight:500, whiteSpace:'nowrap' }}>Select Agent</Typography>
          <Box
          ...
          >
            ...
            <AgentNavIcon />
            <Typography sx={{ fontSize:'0.78rem', color:'#bbb', fontWeight:500, whiteSpace:'nowrap' }}>{selectedAgent.name}</Typography>
        ...
        {/* Archive button */}
        <Box onClick={onOpenArchive} sx={{ position:'relative', display:'flex', alignItems:'center', justifyContent:'center', width:36, height:36, borderRadius:'9px', border:`1px solid ${BORDER}`, color:'#666', cursor:'pointer', transition:'all 0.15s', '&:hover':{ border:`1px solid ${AGENT_COLOR}44`, color:AGENT_COLOR, bgcolor:`${AGENT_COLOR}08` } }}>
          <ArchiveIcon />
          {archivedCount > 0 && <Box sx={{ position:'absolute', top:-4, right:-4, width:16, height:16, borderRadius:'50%', bgcolor:AGENT_COLOR, display:'flex', alignItems:'center', justifyContent:'center' }}><Typography sx={{ fontSize:'0.55rem', fontWeight:800, color:'#fff' }}>{archivedCount}</Typography></Box>}
        </Box>

        {/* ── CHANGED: User dropdown avatar now uses blue agent theme ── */}
        <Box ref={dropdownRef} sx={{ position:'relative' }}>
          <Box onClick={() => setUserOpen(p => !p)} sx={{ display:'flex', alignItems:'center', gap:1, px:1.2, py:0.6, borderRadius:'9px', border:`1px solid ${userOpen?'#2a2a2a':BORDER}`, bgcolor:userOpen?'#111':'transparent', cursor:'pointer', transition:'all 0.15s', '&:hover':{ border:`1px solid #2a2a2a`, bgcolor:'#111' } }}>
            <Box sx={{ width:26, height:26, borderRadius:'50%', background:`linear-gradient(135deg, ${AGENT_COLOR} 0%, #1d4ed8 100%)`, border:`1px solid ${AGENT_COLOR}60`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><Typography sx={{ fontSize:'0.65rem', fontWeight:800, color:'#fff' }}>U</Typography></Box>
          ...
          {userOpen && (
            <Box sx={{ position:'absolute', top:'calc(100% + 8px)', right:0, bgcolor:'#161616', border:`1px solid #2a2a2a`, borderRadius:'10px', py:0.5, minWidth:160, boxShadow:'0 8px 32px rgba(0,0,0,0.7)', zIndex:200 }}>
              <Box sx={{ px:1.8, py:0.8, mb:0.5 }}><Typography sx={{ fontSize:'0.72rem', fontWeight:600, color:'#fff' }}>User Name</Typography><Typography sx={{ fontSize:'0.65rem', color:'#555' }}>user@commedia.ai</Typography></Box>
              <Box sx={{ height:'1px', bgcolor:'#1a1a1a', mx:1, mb:0.5 }} />
              <Box sx={{ px:1.8, py:1, display:'flex', alignItems:'center', gap:1, cursor:'pointer', '&:hover':{ bgcolor:'#1e1e1e' } }} onClick={() => setUserOpen(false)}><SettingsIcon /><Typography sx={{ fontSize:'0.78rem', color:'#ccc' }}>Settings</Typography></Box>
              <Box sx={{ height:'1px', bgcolor:'#1a1a1a', mx:1, my:0.3 }} />
              <Box sx={{ px:1.8, py:1, display:'flex', alignItems:'center', gap:1, cursor:'pointer', '&:hover':{ bgcolor:'#1e0808' } }} onClick={() => router.push('/')}><LogoutIcon /><Typography sx={{ fontSize:'0.78rem', color:'#ef4444' }}>Log out</Typography></Box>
            </Box>
          )}
        </Box>
      </Box>
...
export default function AgentChatPage() {
  ...
  return (
    <Box sx={{ display:'flex', width:'100vw', height:'100vh', bgcolor:'background.default', color:'text.primary', overflow:'hidden', transition:'background-color 0.3s ease, color 0.3s ease' }} onDragStart={e => { const id = (e.target as HTMLElement).closest('[draggable]')?.getAttribute('data-id'); if (id) setDraggingId(id); }} onDragEnd={() => setDraggingId(null)}>
      ...
      <Box sx={{ flex:1, display:'flex', flexDirection:'column', height:'100vh', minWidth:0 }}>
        <ChatNavbar title={active.title} archivedCount={archivedSessions.length} onOpenArchive={() => setArchiveModal(true)} currentAgentId={2} />
