...
const AGENT_COLOR = '#ef4444'; // Red for Monitor Agent
...
function AgentIconSm() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>; }
function AgentIconLg() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>; }
...
function AgentNavIcon() { return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>; }
...
const AGENT_NAME       = 'Monitor Agent';
const AGENT_SHORT      = 'Monitor';
const AGENT_SUBTITLE   = 'Real-time device monitoring & alert management';
const AGENT_SYSTEM     = `You are an expert network monitoring AI agent called "Monitor Agent" for Commedia AI Operations. You provide real-time visibility into network devices — health metrics, critical alerts, capacity tracking, offline devices, and alert trends. Be concise, technical, and structured. Use bullet points and **bold** for key terms.`;
const CAPABILITIES     = ["Real-time device monitoring", "Critical alert management", "Capacity & health tracking", "Offline device detection"];
const PROMPTS          = ["Show all critical alerts", "Check device health summary", "List offline devices", "Analyze capacity trends", "Monitor core switch status", "Show alert history"];
const PLACEHOLDER      = 'Ask about devices, alerts, or network health...';
const INITIAL_CONTENT  = `Hi! I'm your **Monitor Agent**. Real-time device monitoring & alert management.\n\nHow can I help you today?`;
    ...
    <span key={i}>
      {line.split(/(\*\*[^*]+\*\*)/g).map((p, j) =>
        p.startsWith('**') && p.endsWith('**')
          ? <strong key={j} style={{ color: 'inherit', fontWeight: 700 }}>{p.slice(2,-2)}</strong>
          : p
      )}
      {i < arr.length - 1 && <br />}
    </span>
...
function Bubble({ msg }: { msg: Message }) {
  ...
  const CARD_BG = isDark ? '#111' : '#ffffff';
  ...
  return (
    <Box sx={{ display:'flex', flexDirection:isUser?'row-reverse':'row', alignItems:'flex-end', gap:1.2, mb:2 }}>
      ...
      {isUser  && <Box sx={{ width:30, height:30, borderRadius:'9px', background:`linear-gradient(135deg, ${AGENT_COLOR} 0%, #000 100%)`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, mb:0.2 }}><Typography sx={{ fontSize:'0.7rem', fontWeight:800, color:'#fff' }}>U</Typography></Box>}
      <Box sx={{ maxWidth:'70%' }}>
        <Box sx={{ px:2, py:1.5, borderRadius:isUser?'14px 14px 4px 14px':'14px 14px 14px 4px', background:isUser?`linear-gradient(135deg, ${AGENT_COLOR}22, ${AGENT_COLOR}12)`:CARD_BG, border:`1px solid ${isUser?AGENT_COLOR+'35':BORDER}`, fontSize:'0.83rem', color:isDark?'#ccc':'#111827', lineHeight:1.65 }}>
          {renderContent(msg.content)}
        </Box>
        {msg.ts && <Typography sx={{ fontSize:'0.62rem', color:isDark?'#444':'#6b7280', mt:0.4, textAlign:isUser?'right':'left' }}>{msg.ts}</Typography>}
      </Box>
    </Box>
  );
}
...
function PopupMenu({ x, y, items, onClose }: { x: number; y: number; items: PopupItem[]; onClose: () => void }) {
  ...
  return (
    <Box onClick={e => e.stopPropagation()} sx={{ position:'fixed', top:y, left:x, zIndex:9999, background:isDark?'#161616':'#ffffff', border:`1px solid ${isDark?'#2a2a2a':'#e5e7eb'}`, borderRadius:'10px', py:0.5, minWidth:170, boxShadow:isDark?'0 8px 32px rgba(0,0,0,0.7)':'0 8px 24px rgba(0,0,0,0.1)' }}>
      {items.map(item => (
        <Box key={item.label} onClick={() => { item.action(); onClose(); }} sx={{ display:'flex', alignItems:'center', gap:1.2, px:1.8, py:1, cursor:'pointer', transition:'background 0.12s', '&:hover':{ background:isDark?'#1e1e1e':'#f9fafb' } }}>
          <Box sx={{ color:item.color||(isDark?'#ccc':'#6b7280'), display:'flex' }}>{item.icon}</Box>
          <Typography sx={{ fontSize:'0.78rem', color:item.color||(isDark?'#ccc':'#111827') }}>{item.label}</Typography>
        </Box>
      ))}
    </Box>
  );
...
function MoveToThreadPicker({ x, y, threads, onMove, onClose }: { x:number; y:number; threads:Thread[]; onMove:(threadId:string)=>void; onClose:() => void }) {
  ...
  return (
    <Box onClick={e => e.stopPropagation()} sx={{ position:'fixed', top:y, left:x, zIndex:10000, background:isDark?'#161616':'#ffffff', border:`1px solid ${isDark?'#2a2a2a':'#e5e7eb'}`, borderRadius:'10px', py:0.5, minWidth:180, boxShadow:isDark?'0 8px 32px rgba(0,0,0,0.7)':'0 8px 24px rgba(0,0,0,0.1)' }}>
      <Typography sx={{ fontSize:'0.65rem', fontWeight:700, color:isDark?'#444':'#9ca3af', letterSpacing:'0.1em', textTransform:'uppercase', px:1.8, py:0.8 }}>Move to Thread</Typography>
      {threads.length === 0
        ? <Typography sx={{ fontSize:'0.75rem', color:isDark?'#444':'#9ca3af', px:1.8, py:1 }}>No threads yet</Typography>
        : threads.map(t => (
          <Box key={t.id} onClick={() => { onMove(t.id); onClose(); }} sx={{ display:'flex', alignItems:'center', gap:1.2, px:1.8, py:1, cursor:'pointer', '&:hover':{ background:isDark?'#1e1e1e':'#f9fafb' } }}>
            <Box sx={{ color:'#8b5cf6', display:'flex' }}><ThreadIcon /></Box>
            <Typography sx={{ fontSize:'0.78rem', color:isDark?'#ccc':'#111827' }}>{t.name}</Typography>
          </Box>
        ))
      }
    </Box>
      ...
      <Box sx={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:480, bgcolor:'#141414', border:`1px solid #2a2a2a`, borderRadius:'16px', p:3, outline:'none', boxShadow:'0 24px 64px rgba(0,0,0,0.8)', maxHeight:'70vh', display:'flex', flexDirection:'column' }}>
        ...
        <Box sx={{ flex:1, overflowY:'auto', '&::-webkit-scrollbar':{ display:'none' } }}>
          {sessions.length === 0
            ...
            : sessions.map(s => (
              <Box key={s.id} sx={{ display:'flex', alignItems:'center', gap:1.5, px:1.5, py:1.2, borderRadius:'10px', bgcolor:'#0f0f0f', border:`1px solid #1a1a1a`, mb:1, '&:hover':{ bgcolor:'#111' }, transition:'background 0.15s' }}>
...
function ChatNavbar({ title, archivedCount, onOpenArchive }: { title: string; archivedCount: number; onOpenArchive: () => void }) {
  ...
  const derivedAgentId = Number(Object.entries(AGENT_ROUTES).find(([, path]) => pathname?.startsWith(path))?.[0]) || 1;
  const selectedAgent = AGENTS.find(a => a.id === derivedAgentId) ?? AGENTS[0];

  const agentDropdownSx = {
    background: isDark ? '#0f0f0f' : '#ffffff', border: `1px solid ${isDark ? '#2a2a2a' : '#e5e7eb'}`, borderRadius: '10px', mt: 1, minWidth: 170,
    boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.6)' : '0 8px 24px rgba(0,0,0,0.1)',
    '& .MuiMenuItem-root': {
      color: isDark ? '#bbb' : '#4b5563', fontSize: '0.8rem', py: 1, px: 1.8, gap: 1.2, transition: 'background 0.15s',
      '&:hover': { background: isDark ? '#1a1a1a' : '#f9fafb', color: isDark ? '#fff' : '#111827' },
      '&.Mui-selected': { background: isDark ? '#1e1e1e' : '#f3f4f6' },
    },
  };

  useEffect(() => {
    if (!userOpen) return;
    const handler = (e: MouseEvent) => { if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) setUserOpen(false); };
  ...
  return (
    <Box sx={{ px:3, borderBottom:`1px solid ${BORDER}`, display:'flex', alignItems:'center', justifyContent:'space-between', minHeight:64, flexShrink:0 }}>
      <Box>
        <Typography sx={{ fontSize:'0.88rem', fontWeight:700, color:TEXT_MAIN }}>{title}</Typography>
        <Box sx={{ display:'flex', alignItems:'center', gap:0.6, mt:0.2 }}>
          ...
          <Typography sx={{ fontSize:'0.68rem', color:TEXT_MUTED }}>{AGENT_NAME} · Ready</Typography>
        </Box>
      </Box>

      <Box sx={{ display:'flex', alignItems:'center', gap:1 }}>
        <Box sx={{ display:'flex', alignItems:'center', gap:0.8 }}>
          <Typography sx={{ fontSize:'0.7rem', color:TEXT_MUTED, fontWeight:500, whiteSpace:'nowrap' }}>Select Agent</Typography>
          <Box onClick={(e) => setAgentAnchor(e.currentTarget)} sx={{ display:'flex', alignItems:'center', gap:1, px:1.4, py:0.65, borderRadius:'9px', border:`1px solid ${BORDER}`, cursor:'pointer', transition:'all 0.15s', '&:hover':{ border:`1px solid ${selectedAgent.color}44`, bgcolor:`${selectedAgent.color}08` } }}>
            ...
            <AgentNavIcon />
            <Typography sx={{ fontSize:'0.78rem', color:isDark?'#bbb':'#4b5563', fontWeight:500, whiteSpace:'nowrap' }}>{selectedAgent.name}</Typography>
            <Box sx={{ color:TEXT_MUTED, display:'flex' }}><ChevronDownIcon /></Box>
          </Box>
        </Box>
        
        <Menu anchorEl={agentAnchor} open={Boolean(agentAnchor)} onClose={() => setAgentAnchor(null)} PaperProps={{ sx: agentDropdownSx }}>
          {AGENTS.map((a) => (
            <MenuItem key={a.id} selected={a.id === selectedAgent.id} onClick={() => { setAgentAnchor(null); if (a.id !== selectedAgent.id) router.push(AGENT_ROUTES[a.id]); }}>
        ...
        <Box onClick={onOpenArchive} sx={{ position:'relative', display:'flex', alignItems:'center', justifyContent:'center', width:36, height:36, borderRadius:'9px', border:`1px solid ${BORDER}`, color:TEXT_MUTED, cursor:'pointer', transition:'all 0.15s', '&:hover': { border:`1px solid ${AGENT_COLOR}44`, color:AGENT_COLOR, bgcolor:`${AGENT_COLOR}08` } }}>
          <ArchiveIcon />
          {archivedCount > 0 && (
        ...
        <Box ref={dropdownRef} sx={{ position:'relative' }}>
          <Box onClick={() => setUserOpen(p => !p)} sx={{ display:'flex', alignItems:'center', gap:1, px:1.2, py:0.6, borderRadius:'9px', border:`1px solid ${userOpen?(isDark?'#2a2a2a':'#d1d5db'):BORDER}`, bgcolor:userOpen?(isDark?'#111':'#f9fafb'):'transparent', cursor:'pointer', transition:'all 0.15s', '&:hover':{ border:`1px solid ${isDark?'#2a2a2a':'#d1d5db'}`, bgcolor:isDark?'#111':'#f3f4f6' } }}>
            <Box sx={{ width:26, height:26, borderRadius:'50%', background:`linear-gradient(135deg, ${AGENT_COLOR} 0%, #000 100%)`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              <Typography sx={{ fontSize:'0.65rem', fontWeight:800, color:'#fff' }}>U</Typography>
            </Box>
            <Box sx={{ display:{ xs:'none', sm:'block' } }}>
              <Typography sx={{ fontSize:'0.75rem', fontWeight:600, color:isDark?'#ddd':'#111827', lineHeight:1.2 }}>User Name</Typography>
              <Typography sx={{ fontSize:'0.62rem', color:TEXT_MUTED }}>Admin</Typography>
            </Box>
            <Box sx={{ color:TEXT_MUTED, display:'flex' }}><ChevronUserIcon /></Box>
          </Box>
        </Box>
      </Box>
    </Box>
    ...
    <Box onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={e => { e.preventDefault(); const id = e.dataTransfer.getData('sessionId'); if (id) onDropOnThread(thread.id, id); setDragOver(false); }} sx={{ mb:0.5 }}>
      <Box sx={{ display:'flex', alignItems:'center', gap:0.5, px:1, py:0.8, borderRadius:'8px', bgcolor:dragOver?`${AGENT_COLOR}10`:'transparent', border:`1px solid ${dragOver?AGENT_COLOR+'30':'transparent'}`, transition:'all 0.15s', '&:hover .thread-dots':{ opacity:1 }, '&:hover':{ bgcolor:'#0d0d0d' } }}>
        ...
        <Box sx={{ flex:1, minWidth:0, ml:0.8, cursor:'pointer' }} onClick={onToggle}>
          {renamingThread
            ? <input autoFocus value={renameThreadVal} onChange={e => setRenameThreadVal(e.target.value)} onBlur={() => { onRename(); setRenamingThread(false); }} onKeyDown={e => { if (e.key==='Enter') { onRename(); setRenamingThread(false); } e.stopPropagation(); }} style={{ background:'transparent', border:'none', outline:'none', color:'#ccc', fontSize:'0.78rem', width:'100%', fontFamily:'inherit' }} />
      ...
      {thread.expanded && (
        <Box sx={{ ml:2, borderLeft:`1px solid #1a1a1a`, pl:1, mt:0.3 }}>
...
export default function AgentChatPage() {
  ...
  function newSession() { const s = makeSession(getNow()); setSessions(p => [s,...p]); setActiveId(s.id); }
    ...
    <Box sx={{ display:'flex', width:'100vw', height:'100vh', overflow:'hidden', }} onDragStart={e => { const id = (e.target as HTMLElement).closest('[draggable]')?.getAttribute('data-id'); if (id) setDraggingId(id); }} onDragEnd={() => setDraggingId(null)}>
      ...
      <Box sx={{ width:260, flexShrink:0, borderRight:`1px solid ${BORDER}`, display:'flex', flexDirection:'column', height:'100vh', bgcolor:isDark?'transparent':'#ffffff' }}>
        <Box sx={{ px:2, py:2, borderBottom:`1px solid ${BORDER}`, display:'flex', alignItems:'center', gap:1.5, minHeight:64 }}>
          <Box onClick={() => router.push('/pages/dashboard')} sx={{ display:'flex', alignItems:'center', justifyContent:'center', width:30, height:30, borderRadius:'8px', border:`1px solid ${BORDER}`, color:TEXT_MUTED, cursor:'pointer', transition:'all 0.15s', flexShrink:0, '&:hover':{ border:`1px solid ${isDark?'#333':'#d1d5db'}`, color:TEXT_MAIN, bgcolor:isDark?'#141414':'#f9fafb' } }}><BackIcon /></Box>
        ...
        <Box sx={{ flex:1, display:'flex', flexDirection:'column', minHeight:0, overflow:'hidden' }}>
          {threads.length > 0 && (
            <Box sx={{ flexShrink:0, px:1.5, pt:1 }}>
              <Typography sx={{ fontSize:'0.6rem', fontWeight:700, color:TEXT_MUTED, letterSpacing:'0.1em', textTransform:'uppercase', px:0.5, mb:1, mt:0.5 }}>Threads</Typography>
              {threads.map(t => (
          ...
          <Box sx={{ flex:1, overflowY:'auto', px:1.5, pb:1, '&::-webkit-scrollbar':{ width:3 }, '&::-webkit-scrollbar-thumb':{ bgcolor:isDark?'#2a2a2a':'#cbd5e1', borderRadius:4 } }}>
            <Typography sx={{ fontSize:'0.6rem', fontWeight:700, color:TEXT_MUTED, letterSpacing:'0.1em', textTransform:'uppercase', px:0.5, mb:1, mt:1 }}>Recent Chats</Typography>
        ...
        <Box sx={{ px:2, pt:1.5, pb:2, borderTop:`1px solid ${BORDER}`, flexShrink:0 }}>
          <Typography sx={{ fontSize:'0.6rem', fontWeight:700, color:TEXT_MUTED, letterSpacing:'0.1em', textTransform:'uppercase', mb:1 }}>Capabilities</Typography>
          {CAPABILITIES.map((c: string) => <Box key={c} sx={{ display:'flex', alignItems:'center', gap:0.8, mb:0.2 }}><Box sx={{ width:4, height:4, borderRadius:'50%', bgcolor:AGENT_COLOR, flexShrink:0 }} /><Typography sx={{ fontSize:'0.7rem', color:TEXT_MUTED }}>{c}</Typography></Box>)}
        </Box>
      </Box>
      ...
      <Box sx={{ flex:1, display:'flex', flexDirection:'column', height:'100vh', minWidth:0 }}>
        ...
        <Box sx={{ flex:1, overflowY:'auto', px:4, py:3, '&::-webkit-scrollbar':{ width:4 }, '&::-webkit-scrollbar-thumb':{ bgcolor:isDark?'#1a1a1a':'#cbd5e1', borderRadius:2 } }}>
          {active.messages.length === 1 && (
            <Box sx={{ textAlign:'center', mb:4, mt:2 }}>
              ...
              <Typography sx={{ fontSize:'1.1rem', fontWeight:800, color:TEXT_MAIN, mb:0.5 }}>{AGENT_NAME}</Typography>
              <Typography sx={{ fontSize:'0.78rem', color:TEXT_MUTED, mb:3 }}>{AGENT_SUBTITLE}</Typography>
              <Box sx={{ display:'flex', flexWrap:'wrap', gap:1, justifyContent:'center', maxWidth:560, mx:'auto' }}>
                {PROMPTS.map((p: string) => <Box key={p} onClick={() => send(p)} sx={{ px:1.8, py:0.9, borderRadius:'20px', border:`1px solid ${BORDER}`, bgcolor:CARD_BG, cursor:'pointer', transition:'all 0.15s', '&:hover':{ border:`1px solid ${AGENT_COLOR}44`, bgcolor:`${AGENT_COLOR}08` } }}><Typography sx={{ fontSize:'0.75rem', color:isDark?'#888':'#4b5563' }}>{p}</Typography></Box>)}
              </Box>
            </Box>
          )}
          {active.messages.map((m,i) => <Bubble key={i} msg={m} />)}
          {loading && <Box sx={{ display:'flex', alignItems:'flex-end', gap:1.2, mb:2 }}><Box sx={{ width:30, height:30, borderRadius:'9px', bgcolor:`${AGENT_COLOR}20`, border:`1px solid ${AGENT_COLOR}40`, display:'flex', alignItems:'center', justifyContent:'center', color:AGENT_COLOR, flexShrink:0 }}><AgentIconSm /></Box><Box sx={{ px:2, py:1.5, borderRadius:'14px 14px 4px 14px', background:isDark?'#111':'#ffffff', border:`1px solid ${BORDER}` }}><TypingDots /></Box></Box>}
          <div ref={bottomRef} />
        </Box>

        <Box sx={{ px:4, py:2.5, borderTop:`1px solid ${BORDER}`, flexShrink:0, bgcolor:isDark?'transparent':'#ffffff' }}>
          <TextField fullWidth multiline maxRows={4} placeholder={PLACEHOLDER} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();} }}
            sx={{ '& .MuiOutlinedInput-root':{ bgcolor:isDark?'#111':'#f9fafb', borderRadius:'12px', color:TEXT_MAIN, fontSize:'0.85rem', '& textarea':{ '&::placeholder':{ color:TEXT_MUTED, opacity:1 } }, '& fieldset':{ border:`1px solid ${BORDER}` }, '&:hover fieldset':{ border:`1px solid ${isDark?'#2a2a2a':'#d1d5db'}` }, '&.Mui-focused fieldset':{ border:`1px solid ${AGENT_COLOR}55` } } }}
            InputProps={{ endAdornment:(
              <InputAdornment position="end">
                <IconButton onClick={() => send()} disabled={!input.trim()||loading} sx={{ width:34, height:34, bgcolor:input.trim()&&!loading?AGENT_COLOR:(isDark?'#1a1a1a':'#e5e7eb'), borderRadius:'9px', color:input.trim()&&!loading?'#fff':TEXT_MUTED, transition:'all 0.2s', '&:hover':{ filter:input.trim()&&!loading?'brightness(1.1)':'none' } }}><SendIcon /></IconButton>
              </InputAdornment>
            )}}
          />
          <Typography sx={{ fontSize:'0.65rem', color:TEXT_MUTED, textAlign:'center', mt:1 }}>Commedia AI · Press Enter to send, Shift+Enter for new line</Typography>
        </Box>
      </Box>
