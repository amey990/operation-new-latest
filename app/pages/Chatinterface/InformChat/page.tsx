...
function AgentIconSm() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/></svg>; }
function AgentIconLg() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/></svg>; }
...
const AGENT_NAME       = 'Inform Agent';
const AGENT_SHORT      = 'Inform';
const AGENT_SUBTITLE   = 'Stakeholder notifications & incident reporting';
const AGENT_SYSTEM     = `You are an expert notification AI agent called "Inform Agent" for Commedia AI Operations. You help engineers send stakeholder notifications, generate incident reports, manage communication templates, and track acknowledgements. Be clear, professional, and concise. Use **bold** for key details.`;
const CAPABILITIES     = ["Stakeholder notifications", "Incident report generation", "Communication templates", "Acknowledgement tracking"];
const PROMPTS          = ["Send P1 incident alert", "Draft incident report", "Notify all stakeholders", "Create status update", "Schedule maintenance notice", "Track notification delivery"];
const PLACEHOLDER      = 'Describe your notification or report...';
const INITIAL_CONTENT  = `Hi! I'm your **Inform Agent**. Stakeholder notifications & incident reporting.\n\nHow can I help you today?`;
...
function Bubble({ msg }: { msg: Message }) {
  ...
  return (
    <Box sx={{ display:'flex', flexDirection:isUser?'row-reverse':'row', alignItems:'flex-end', gap:1.2, mb:2 }}>
      ...
      {/* ── CHANGED: user bubble avatar now uses yellow agent theme ── */}
      {isUser && <Box sx={{ width:30, height:30, borderRadius:'9px', background:`linear-gradient(135deg, ${AGENT_COLOR} 0%, #d97706 100%)`, border:`1px solid ${AGENT_COLOR}60`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, mb:0.2 }}><Typography sx={{ fontSize:'0.7rem', fontWeight:800, color:'#000' }}>U</Typography></Box>}
      <Box sx={{ maxWidth:'70%' }}>
...
function ChatNavbar({ title, archivedCount, onOpenArchive }: { title: string; archivedCount: number; onOpenArchive: () => void }) {
  ...
  const derivedAgentId =
    Number(Object.entries(AGENT_ROUTES).find(([, path]) => pathname.startsWith(path))?.[0]) || 4;

  const selectedAgent = AGENTS.find(a => a.id === derivedAgentId) ?? AGENTS[3];
  ...
  return (
    <Box sx={{ px: 3, borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', minHeight: 64, flexShrink: 0 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        {/* ── CHANGED: User dropdown avatar now uses yellow agent theme ── */}
        <Box ref={dropdownRef} sx={{ position: 'relative' }}>
          <Box onClick={() => setUserOpen(p => !p)} sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.2, py: 0.6, borderRadius: '9px', border: `1px solid ${userOpen ? '#2a2a2a' : BORDER}`, bgcolor: userOpen ? '#111' : 'transparent', cursor: 'pointer', transition: 'all 0.15s', '&:hover': { border: `1px solid #2a2a2a`, bgcolor: '#111' } }}>
            <Box sx={{ width: 26, height: 26, borderRadius: '50%', background: `linear-gradient(135deg, ${AGENT_COLOR} 0%, #d97706 100%)`, border: `1px solid ${AGENT_COLOR}60`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <TextField fullWidth multiline maxRows={4} placeholder={PLACEHOLDER} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();} }}
            ...
            InputProps={{ endAdornment:(
              <InputAdornment position="end">
                <IconButton onClick={() => send()} disabled={!input.trim()||loading} sx={{ width:34, height:34, bgcolor:input.trim()&&!loading?AGENT_COLOR:'#1a1a1a', borderRadius:'9px', color:input.trim()&&!loading?'#000':'#444', transition:'all 0.2s', '&:hover':{ filter:input.trim()&&!loading?'brightness(1.1)':'none' }, '&.Mui-disabled':{ bgcolor:'#1a1a1a', color:'#333' } }}><SendIcon /></IconButton>
              </InputAdornment>
            )}}
          />
