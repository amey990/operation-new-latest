...
function AgentIconSm() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/><path d="M6 6h.01M6 18h.01"/></svg>; }
function AgentIconLg() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/><path d="M6 6h.01M6 18h.01"/></svg>; }
...
const AGENT_NAME       = 'Packet Agent';
const AGENT_SHORT      = 'Packet';
const AGENT_SUBTITLE   = 'Packet analysis & traffic anomaly detection';
const AGENT_SYSTEM     = `You are an expert packet analysis AI agent called "Packet Agent" for Commedia AI Operations. You analyze network traffic, detect anomalies, inspect VLAN traffic, identify bandwidth issues, and monitor QoS metrics. Be technical and precise. Use bullet points and **bold** for key terms.`;
const CAPABILITIES     = ["Packet capture analysis", "Traffic anomaly detection", "VLAN traffic inspection", "Bandwidth & QoS monitoring"];
const PROMPTS          = ["Analyze VLAN 20 traffic", "Detect traffic anomalies", "Check bandwidth usage", "Inspect packet loss", "Monitor QoS metrics", "Show top talkers"];
const PLACEHOLDER      = 'Describe your traffic analysis task...';
const INITIAL_CONTENT  = `Hi! I'm your **Packet Agent**. Packet analysis & traffic anomaly detection.\n\nHow can I help you today?`;
...
function Bubble({ msg }: { msg: Message }) {
  ...
  return (
    <Box sx={{ display:'flex', flexDirection:isUser?'row-reverse':'row', alignItems:'flex-end', gap:1.2, mb:2 }}>
      ...
      {/* ── CHANGED: user bubble avatar now uses pink agent theme ── */}
      {isUser && <Box sx={{ width:30, height:30, borderRadius:'9px', background:`linear-gradient(135deg, ${AGENT_COLOR} 0%, #be185d 100%)`, border:`1px solid ${AGENT_COLOR}60`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, mb:0.2 }}><Typography sx={{ fontSize:'0.7rem', fontWeight:800, color:'#fff' }}>U</Typography></Box>}
      <Box sx={{ maxWidth:'70%' }}>
...
function ChatNavbar({ title, archivedCount, onOpenArchive }: { title: string; archivedCount: number; onOpenArchive: () => void }) {
  ...
  const derivedAgentId =
    Number(Object.entries(AGENT_ROUTES).find(([, path]) => pathname.startsWith(path))?.[0]) || 6;

  const selectedAgent = AGENTS.find(a => a.id === derivedAgentId) ?? AGENTS[5];
  ...
  return (
    <Box sx={{ px: 3, borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', minHeight: 64, flexShrink: 0 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        {/* ── CHANGED: User dropdown avatar now uses pink agent theme ── */}
        <Box ref={dropdownRef} sx={{ position: 'relative' }}>
          <Box onClick={() => setUserOpen(p => !p)} sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.2, py: 0.6, borderRadius: '9px', border: `1px solid ${userOpen ? '#2a2a2a' : BORDER}`, bgcolor: userOpen ? '#111' : 'transparent', cursor: 'pointer', transition: 'all 0.15s', '&:hover': { border: `1px solid #2a2a2a`, bgcolor: '#111' } }}>
            <Box sx={{ width: 26, height: 26, borderRadius: '50%', background: `linear-gradient(135deg, ${AGENT_COLOR} 0%, #be185d 100%)`, border: `1px solid ${AGENT_COLOR}60`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
