...
function TroubleshootIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>; }
function TroubleshootIconSm() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>; }
...
// ── Agent route map (MATCH your app folder structure exactly) ─────────
const AGENT_ROUTES: Record<number, string> = {
  1: '/pages/Chatinterface/MonitorChat',
  2: '/pages/Chatinterface/TicketChat',
  3: '/pages/Chatinterface/Troubleshootchat', // match folder name exactly
  4: '/pages/Chatinterface/InformChat',
  5: '/pages/Chatinterface/ConfigureChat',
  6: '/pages/Chatinterface/PacketChat',
};

const PROMPTS = ['Diagnose a connectivity issue','Check switch & AP status','Run a ping test','Generate an RCA report','Analyze network topology','Troubleshoot VLAN issue'];
const INITIAL_CONTENT = "Hi! I'm your **Troubleshoot Agent**. I can help diagnose network issues, run connectivity tests, analyze switch and AP status, and generate RCA reports.\n\nWhat would you like to troubleshoot today?";
...
function useClientTs() {
  const [ts, setTs] = useState('');
  useEffect(() => { setTs(getNow()); }, []);
  return ts;
}
...
function TypingDots() {
  return (
    <Box sx={{ display:'flex', alignItems:'center', gap:0.5, py:0.5 }}>
      {[0,1,2].map(i => <Box key={i} sx={{ width:6, height:6, borderRadius:'50%', bgcolor:GREEN, animation:'pulse 1.2s ease-in-out infinite', animationDelay:`${i*0.2}s`, '@keyframes pulse':{'0%,100%':{opacity:0.3,transform:'scale(0.8)'},'50%':{opacity:1,transform:'scale(1)'}} }} />)}
    </Box>
  );
}
...
function Bubble({ msg }: { msg: Message }) {
  ...
  return (
    <Box sx={{ display:'flex', flexDirection:isUser?'row-reverse':'row', alignItems:'flex-end', gap:1.2, mb:2 }}>
      {!isUser && <Box sx={{ width:30, height:30, borderRadius:'9px', bgcolor:`${GREEN}20`, border:`1px solid ${GREEN}40`, display:'flex', alignItems:'center', justifyContent:'center', color:GREEN, flexShrink:0, mb:0.2 }}><TroubleshootIconSm /></Box>}
      {isUser  && <Box sx={{ width:30, height:30, borderRadius:'9px', background:'linear-gradient(135deg, #3ECF8E 0%, #18a860 100%)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, mb:0.2 }}><Typography sx={{ fontSize:'0.7rem', fontWeight:800, color:'#000' }}>U</Typography></Box>}
      <Box sx={{ maxWidth:'70%' }}>
        <Box sx={{ px:2, py:1.5, borderRadius:isUser?'14px 14px 4px 14px':'14px 14px 14px 4px', background:isUser?`linear-gradient(135deg, ${GREEN}22, ${GREEN}12)`:CARD2, border:`1px solid ${isUser?GREEN+'35':BORDER}`, fontSize:'0.83rem', color:'#ccc', lineHeight:1.65 }}>
    ...
    <Modal open={open} onClose={onClose}>
      <Box sx={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:420, bgcolor:'#141414', border:`1px solid #2a2a2a`, borderRadius:'16px', p:3, outline:'none', boxShadow:'0 24px 64px rgba(0,0,0,0.8)' }}>
        ...
        <TextField fullWidth autoFocus placeholder="e.g. Site B Outage, Core Switch Issues..." value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key==='Enter'&&submit()}
          sx={{ mb:2, '& .MuiOutlinedInput-root':{ bgcolor:'#0f0f0f', borderRadius:'10px', fontSize:'0.85rem', '& input':{ color:'#fff', '&::placeholder':{ color:'#555', opacity:1 } }, '& fieldset':{ border:`1px solid #2a2a2a` }, '&:hover fieldset':{ border:`1px solid #333` }, '&.Mui-focused fieldset':{ border:`1px solid ${GREEN}55` } } }}
        />
        <Box sx={{ display:'flex', gap:1.2, p:1.5, bgcolor:'#0f0f0f', borderRadius:'10px', border:`1px solid #1a1a1a`, mb:2.5 }}>
          <Box sx={{ color:GREEN, mt:0.1, flexShrink:0 }}><ThreadIcon /></Box>
        ...
        <Box sx={{ display:'flex', gap:1.2, justifyContent:'flex-end' }}>
          ...
          <Box onClick={submit} sx={{ px:2.5, py:0.9, borderRadius:'8px', bgcolor:name.trim()?GREEN:'#1a1a1a', cursor:name.trim()?'pointer':'default', transition:'all 0.15s', '&:hover':{ filter:name.trim()?'brightness(1.1)':'none' } }}>
            <Typography sx={{ fontSize:'0.8rem', fontWeight:700, color:name.trim()?'#000':'#444' }}>Create Thread</Typography>
          </Box>
        </Box>
      </Box>
    </Modal>
      ...
      <Box sx={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:480, bgcolor:'#141414', border:`1px solid #2a2a2a`, borderRadius:'16px', p:3, outline:'none', boxShadow:'0 24px 64px rgba(0,0,0,0.8)', maxHeight:'70vh', display:'flex', flexDirection:'column' }}>
        <Box sx={{ display:'flex', alignItems:'center', justifyContent:'space-between', mb:2.5 }}>
          <Box sx={{ display:'flex', alignItems:'center', gap:1.2 }}>
            ...
            <Box sx={{ px:0.9, py:0.2, borderRadius:'6px', bgcolor:'#1a1a1a' }}>
              <Typography sx={{ fontSize:'0.68rem', color:'#555', fontWeight:600 }}>{sessions.length}</Typography>
            </Box>
          </Box>
          <Box onClick={onClose} sx={{ color:'#555', cursor:'pointer', display:'flex', p:0.4, borderRadius:'6px', '&:hover':{ color:'#ccc', bgcolor:'#1e1e1e' } }}><CloseIcon /></Box>
        </Box>
            ...
            : sessions.map(s => (
              <Box key={s.id} sx={{ display:'flex', alignItems:'center', gap:1.5, px:1.5, py:1.2, borderRadius:'10px', bgcolor:'#0f0f0f', border:`1px solid ${BORDER}`, mb:1, '&:hover':{ bgcolor:'#111' }, transition:'background 0.15s' }}>
                ...
                <Box sx={{ display:'flex', gap:0.5 }}>
                  <Box onClick={() => onRestore(s.id)} sx={{ display:'flex', alignItems:'center', justifyContent:'center', width:28, height:28, borderRadius:'7px', color:GREEN, cursor:'pointer', border:`1px solid transparent`, '&:hover':{ bgcolor:`${GREEN}15`, border:`1px solid ${GREEN}30` } }}>
                    <RestoreIcon />
                  </Box>
                  <Box onClick={() => onDelete(s.id)} sx={{ display:'flex', alignItems:'center', justifyContent:'center', width:28, height:28, borderRadius:'7px', color:'#ef4444', cursor:'pointer', border:`1px solid transparent`, '&:hover':{ bgcolor:'#2e0d0d', border:'1px solid #3d0c0c' } }}>
                    <DeleteIcon />
                  </Box>
                </Box>
              </Box>
...
function ChatNavbar({ title, archivedCount, onOpenArchive }: { title: string; archivedCount: number; onOpenArchive: () => void }) {
  ...
  // detect current agent from route
  const derivedAgentId =
    Number(Object.entries(AGENT_ROUTES).find(([, path]) => pathname?.startsWith(path))?.[0]) || 3;

  const selectedAgent = AGENTS.find(a => a.id === derivedAgentId) ?? AGENTS[2];
    ...
    <Box sx={{ px: 3, borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', minHeight: 64, flexShrink: 0 }}>
      ...
      <Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.6, mt: 0.2 }}>
          <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: GREEN, boxShadow: `0 0 6px ${GREEN}` }} />
          <Typography sx={{ fontSize: '0.68rem', color: '#555' }}>Troubleshoot Agent · Ready</Typography>
        </Box>
      </Box>

      {/* Right: agent selector + archive + user */}
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Box
            ...
            sx={{
              ...
              '&:hover': {
                ...
                bgcolor: `${selectedAgent.color}08`,
              },
            }}
          >
        ...
        >
          {AGENTS.map((a) => (
            <MenuItem
              ...
              onClick={() => {
                setAgentAnchor(null);
                if (a.id !== selectedAgent.id) router.push(AGENT_ROUTES[a.id]);
              }}
            >
        ...
        {/* Archive */}
        <Box onClick={onOpenArchive} sx={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: '9px', border: `1px solid ${BORDER}`, color: '#666', cursor: 'pointer', transition: 'all 0.15s', '&:hover': { border: `1px solid ${GREEN}44`, color: GREEN, bgcolor: `${GREEN}08` } }}>
          <ArchiveIcon />
          {archivedCount > 0 && (
            <Box sx={{ position: 'absolute', top: -4, right: -4, width: 16, height: 16, borderRadius: '50%', bgcolor: GREEN, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Typography sx={{ fontSize: '0.55rem', fontWeight: 800, color: '#000' }}>{archivedCount}</Typography>
            </Box>
          )}
        </Box>
        ...
        <Box ref={dropdownRef} sx={{ position: 'relative' }}>
          <Box onClick={() => setUserOpen(p => !p)} sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.2, py: 0.6, borderRadius: '9px', border: `1px solid ${userOpen ? '#2a2a2a' : BORDER}`, bgcolor: userOpen ? '#111' : 'transparent', cursor: 'pointer', transition: 'all 0.15s', '&:hover': { border: `1px solid #2a2a2a`, bgcolor: '#111' } }}>
            <Box sx={{ width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(135deg, #3ECF8E 0%, #18a860 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Typography sx={{ fontSize: '0.65rem', fontWeight: 800, color: '#000' }}>U</Typography>
            </Box>
...
function ThreadRow({ thread, sessions, activeId, setActiveId, onToggle, onRename, onDelete, onDropOnThread, handleSessionDots, renamingId, renameVal, setRenameVal, setRenamingId, setSessions, draggingId }: {
  ...
  return (
    <Box
      ...
      onDrop={e => { e.preventDefault(); const id = e.dataTransfer.getData('sessionId'); if (id) onDropOnThread(thread.id, id); setDragOver(false); }}
      sx={{ mb: 0.5 }}
    >
      {/* Thread header */}
      <Box sx={{ display:'flex', alignItems:'center', gap:0.5, px:1, py:0.8, borderRadius:'8px', bgcolor:dragOver?`${GREEN}10`:'transparent', border:`1px solid ${dragOver?GREEN+'30':'transparent'}`, transition:'all 0.15s', '&:hover .thread-dots':{ opacity:1 }, '&:hover':{ bgcolor:'#0d0d0d' } }}>
        <Box onClick={onToggle} sx={{ color:'#555', display:'flex', cursor:'pointer', flexShrink:0, '&:hover':{ color:'#888' } }}>
          {thread.expanded ? <ChevronDownIcon /> : <ChevronRightIcon />}
        </Box>
        <Box sx={{ color:'#8b5cf6', display:'flex', flexShrink:0, ml:0.3 }}>
          {thread.expanded ? <ThreadIconOpen /> : <ThreadIcon />}
        </Box>
        <Box sx={{ flex:1, minWidth:0, ml:0.8, cursor:'pointer' }} onClick={onToggle}>
          {renamingThread ? (
            <input autoFocus value={renameThreadVal} onChange={e => setRenameThreadVal(e.target.value)}
              onBlur={() => { onRename(); setSessions(p => p); setRenamingThread(false); }}
              onKeyDown={e => { if (e.key==='Enter') { onRename(); setRenamingThread(false); } e.stopPropagation(); }}
              onClick={e => e.stopPropagation()}
              style={{ background:'transparent', border:'none', outline:'none', color:'#ccc', fontSize:'0.78rem', width:'100%', fontFamily:'inherit' }}
            />
          ) : (
        ...
        {threadSessions.length > 0 && (
          <Box sx={{ px:0.6, py:0.1, borderRadius:'4px', bgcolor:'#1a1a1a', mr:0.5 }}>
            <Typography sx={{ fontSize:'0.6rem', color:'#555' }}>{threadSessions.length}</Typography>
          </Box>
        )}
        <Box className="thread-dots" onClick={e => { e.stopPropagation(); setMenu({ x: e.clientX, y: e.clientY }); }}
          sx={{ opacity:0, display:'flex', alignItems:'center', justifyContent:'center', width:20, height:20, borderRadius:'5px', color:'#666', cursor:'pointer', flexShrink:0, transition:'opacity 0.15s', '&:hover':{ bgcolor:'#2a2a2a', color:'#ccc' } }}>
          <DotsHorizIcon />
        </Box>
      </Box>
      ...
      {thread.expanded && (
        <Box sx={{ ml:2, borderLeft:`1px solid ${BORDER}`, pl:1, mt:0.3 }}>
          {threadSessions.length === 0 ? (
            <Box sx={{ px:1, py:1.5, borderRadius:'7px', border:`1px dashed #222`, textAlign:'center' }}>
              <Typography sx={{ fontSize:'0.68rem', color:'#333' }}>Drop chats here</Typography>
            </Box>
          ) : (
            threadSessions.map(s => (
              <SessionRow key={s.id} s={s} activeId={activeId} setActiveId={setActiveId} handleDots={handleSessionDots}
                renamingId={renamingId} renameVal={renameVal} setRenameVal={setRenameVal} setRenamingId={setRenamingId} setSessions={setSessions} draggingId={draggingId} />
            ))
          )}
        </Box>
      )}
...
function SessionRow({ s, activeId, setActiveId, handleDots, renamingId, renameVal, setRenameVal, setRenamingId, setSessions, draggingId }: {
  ...
  return (
    <Box
      ...
      sx={{ display:'flex', alignItems:'center', gap:0.8, px:1.2, py:0.9, borderRadius:'8px', cursor:'grab', mb:0.3, bgcolor:isActive?`${GREEN}10`:'transparent', border:`1px solid ${isActive?GREEN+'28':'transparent'}`, opacity:isDragging?0.4:1, transition:'all 0.15s', '&:hover':{ bgcolor:isActive?`${GREEN}10`:'#111' }, '&:hover .dots-btn':{ opacity:1 }, '&:hover .drag-handle':{ opacity:1 } }}
    >
      {/* Drag handle */}
      ...
      <Box sx={{ color:isActive?GREEN:'#555', flexShrink:0 }}><ChatIcon /></Box>
      <Box sx={{ flex:1, minWidth:0 }}>
        {renamingId === s.id ? (
          <input autoFocus value={renameVal} onChange={e => setRenameVal(e.target.value)}
            onBlur={() => { setSessions(p => p.map(x => x.id===s.id?{...x,title:renameVal||x.title}:x)); setRenamingId(null); }}
            onKeyDown={e => { if(e.key==='Enter'){setSessions(p=>p.map(x=>x.id===s.id?{...x,title:renameVal||x.title}:x));setRenamingId(null);} e.stopPropagation(); }}
            onClick={e => e.stopPropagation()}
            style={{ background:'transparent', border:'none', outline:'none', color:'#ddd', fontSize:'0.75rem', width:'100%', fontFamily:'inherit' }}
          />
        ) : (
      ...
      <Box className="dots-btn" onClick={e => handleDots(e, s.id)}
        sx={{ opacity:0, display:'flex', alignItems:'center', justifyContent:'center', width:22, height:22, borderRadius:'5px', color:'#666', flexShrink:0, transition:'opacity 0.15s', '&:hover':{ bgcolor:'#2a2a2a', color:'#ccc' } }}>
        <DotsVertIcon />
      </Box>
    </Box>
  );
...
// ── Main ───────────────────────────────────────────────────────────────
export default function TroubleshootChat() {
  const router = useRouter();
  ...
  // Fix hydration: set timestamps client-side
  useEffect(() => {
    if (!initTs) return;
  ...
  function newSession() {
    const s = makeSession(getNow());
    s.messages[0].ts = getNow();
    setSessions(p => [s, ...p]);
    setActiveId(s.id);
  }

  function updateSession(id: string, messages: Message[], title?: string) {
    setSessions(p => p.map(s => s.id === id ? { ...s, messages, title: title ?? s.title } : s));
  }

  function handleDelete(id: string) {
    setSessions(p => { const next = p.filter(s => s.id !== id); if (activeId === id && next[0]) setActiveId(next[0].id); return next; });
  }
  ...
  function handleArchive(id: string) {
    setSessions(p => { const next = p.map(s => s.id === id ? { ...s, archived:true, pinned:false } : s); if (activeId === id) { const n = next.find(s => !s.archived && s.id !== id); if (n) setActiveId(n.id); } return next; });
  }
  function handleRestore(id: string) { setSessions(p => p.map(s => s.id === id ? { ...s, archived:false } : s)); }
  function handleMoveToThread(sessionId: string, threadId: string) {
    setSessions(p => p.map(s => s.id === sessionId ? { ...s, threadId, pinned:false } : s));
    setThreads(p => p.map(t => t.id === threadId ? { ...t, expanded:true } : t));
  }
  function handleDropOnThread(threadId: string, sessionId: string) { handleMoveToThread(sessionId, threadId); setDraggingId(null); }

  function handleSessionDots(e: React.MouseEvent, sessionId: string) {
    e.stopPropagation();
  ...
  function handleThreadRename(threadId: string, name: string) {
    setThreads(p => p.map(t => t.id === threadId ? { ...t, name } : t));
  }
  function handleThreadDelete(threadId: string) {
    setSessions(p => p.map(s => s.threadId === threadId ? { ...s, threadId: undefined } : s));
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          system:`You are an expert network troubleshooting AI agent called "Troubleshoot Agent" for a Network Operations platform called Commedia AI Operations. You help network engineers diagnose and resolve issues including: connectivity problems, switch & AP diagnostics, site topology analysis, ping tests, RCA (Root Cause Analysis) reports, and VLAN issues. Be concise, technical, and structured. Use bullet points and **bold** for key terms. Always ask clarifying questions when needed.`,
          messages: updated.map(m => ({ role:m.role, content:m.content })),
        }),
      });
      const data = await res.json();
      const reply = data.content?.[0]?.text ?? 'Sorry, I could not process that request.';
      updateSession(activeId, [...updated, { role:'assistant', content:reply, ts:getNow() }]);
    } catch {
      updateSession(activeId, [...updated, { role:'assistant', content:'Connection error. Please try again.', ts:getNow() }]);
    ...
    >
      ...
      <Box sx={{ width:260, flexShrink:0, borderRight:`1px solid ${BORDER}`, display:'flex', flexDirection:'column', height:'100vh' }}>
        ...
        <Box sx={{ px:2, py:2, borderBottom:`1px solid ${BORDER}`, display:'flex', alignItems:'center', gap:1.5, minHeight:64 }}>
          <Box onClick={() => router.push('/pages/dashboard')} sx={{ display:'flex', alignItems:'center', justifyContent:'center', width:30, height:30, borderRadius:'8px', border:`1px solid ${BORDER}`, color:'#666', cursor:'pointer', transition:'all 0.15s', flexShrink:0, '&:hover':{ border:`1px solid #333`, color:'#ccc', bgcolor:'#141414' } }}>
            <BackIcon />
          </Box>
          <Box sx={{ display:'flex', alignItems:'center', gap:1, flex:1, minWidth:0 }}>
            <Box sx={{ width:28, height:28, borderRadius:'8px', bgcolor:`${GREEN}20`, border:`1px solid ${GREEN}40`, display:'flex', alignItems:'center', justifyContent:'center', color:GREEN, flexShrink:0 }}>
              <TroubleshootIconSm />
            </Box>
            <Box sx={{ minWidth:0 }}>
              <Typography sx={{ fontSize:'0.8rem', fontWeight:700, color:'#fff', lineHeight:1.2 }}>Troubleshoot</Typography>
              <Typography sx={{ fontSize:'0.62rem', color:'#555' }}>AI Agent</Typography>
            </Box>
          </Box>
        </Box>
        ...
        <Box sx={{ px:2, pt:2, pb:1, display:'flex', flexDirection:'column', gap:0.8 }}>
          <Box onClick={newSession} sx={{ display:'flex', alignItems:'center', gap:1, px:1.5, py:1, borderRadius:'9px', border:`1px solid ${BORDER}`, cursor:'pointer', color:'#888', transition:'all 0.15s', '&:hover':{ border:`1px solid ${GREEN}44`, color:GREEN, bgcolor:`${GREEN}08` } }}>
            <NewChatIcon /><Typography sx={{ fontSize:'0.78rem', fontWeight:500 }}>New Chat</Typography>
          </Box>
          <Box onClick={() => setThreadModal(true)} sx={{ display:'flex', alignItems:'center', gap:1, px:1.5, py:1, borderRadius:'9px', border:`1px solid ${BORDER}`, cursor:'pointer', color:'#888', transition:'all 0.15s', '&:hover':{ border:`1px solid #8b5cf688`, color:'#8b5cf6', bgcolor:'#8b5cf608' } }}>
            <ThreadIcon /><Typography sx={{ fontSize:'0.78rem', fontWeight:500 }}>Create Thread</Typography>
          </Box>
        </Box>
...
<Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
  ...
  {/* Threads — no scroll, always fully shown */}
  {threads.length > 0 && (
  ...
  {/* Pinned + Recent — scrollable */}
  <Box sx={{
    flex: 1, overflowY: 'auto', px: 1.5, pb: 1,
    '&::-webkit-scrollbar': { width: 3 },
    '&::-webkit-scrollbar-track': { bgcolor: 'transparent' },
    '&::-webkit-scrollbar-thumb': { bgcolor: '#2a2a2a', borderRadius: 4, '&:hover': { bgcolor: '#333' } },
  }}>
    {pinnedSessions.length > 0 && (
      <>
        <Typography sx={{ fontSize: '0.6rem', fontWeight: 700, color: '#444', letterSpacing: '0.1em', textTransform: 'uppercase', px: 0.5, mb: 1, mt: 1.5 }}>Pinned</Typography>
        {pinnedSessions.map(s => <SessionRow key={s.id} s={s} activeId={activeId} setActiveId={setActiveId} handleDots={handleSessionDots} renamingId={renamingId} renameVal={renameVal} setRenameVal={setRenameVal} setRenamingId={setRenamingId} setSessions={setSessions} draggingId={draggingId} />)}
      </>
    )}
    ...
    {unpinnedSessions.length === 0 && threads.length === 0 && pinnedSessions.length === 0 && (
      <Typography sx={{ fontSize: '0.72rem', color: '#333', px: 0.5 }}>No chats yet</Typography>
    )}
 ...
 {/* Capabilities — flush at bottom, border perfectly aligned */}
<Box sx={{ px: 2, pt: 1.5, pb: 2, borderTop: `1px solid ${BORDER}`, flexShrink: 0 }}>
  ...
  {['Connectivity diagnosis', 'Switch & AP diagnostics', 'Ping tests & RCA reports', 'VLAN analysis'].map(c => (
    <Box key={c} sx={{ display: 'flex', alignItems: 'center', gap: 0.8, mb: 0.2 }}>
      <Box sx={{ width: 4, height: 4, borderRadius: '50%', bgcolor: GREEN, flexShrink: 0 }} />
      <Typography sx={{ fontSize: '0.7rem', color: '#555' }}>{c}</Typography>
    </Box>
  ))}
</Box>
      </Box>
        ...
        <Box sx={{ flex:1, overflowY:'auto', px:4, py:3, '&::-webkit-scrollbar':{ width:4 }, '&::-webkit-scrollbar-thumb':{ bgcolor:'#1a1a1a', borderRadius:2 } }}>
          {active.messages.length === 1 && (
            <Box sx={{ textAlign:'center', mb:4, mt:2 }}>
              <Box sx={{ width:56, height:56, borderRadius:'16px', bgcolor:`${GREEN}18`, border:`1px solid ${GREEN}35`, display:'flex', alignItems:'center', justifyContent:'center', color:GREEN, mx:'auto', mb:2 }}>
                <TroubleshootIcon />
              </Box>
              <Typography sx={{ fontSize:'1.1rem', fontWeight:800, color:'#fff', mb:0.5 }}>Troubleshoot Agent</Typography>
              <Typography sx={{ fontSize:'0.78rem', color:'#555', mb:3 }}>Diagnose network issues with AI-powered analysis</Typography>
              <Box sx={{ display:'flex', flexWrap:'wrap', gap:1, justifyContent:'center', maxWidth:560, mx:'auto' }}>
                {PROMPTS.map(p => (
                  <Box key={p} onClick={() => send(p)} sx={{ px:1.8, py:0.9, borderRadius:'20px', border:`1px solid ${BORDER}`, bgcolor:CARD, cursor:'pointer', transition:'all 0.15s', '&:hover':{ border:`1px solid ${GREEN}44`, bgcolor:`${GREEN}08` } }}>
                    <Typography sx={{ fontSize:'0.75rem', color:'#888' }}>{p}</Typography>
                  </Box>
                ))}
              </Box>
            </Box>
          ...
          {loading && (
            <Box sx={{ display:'flex', alignItems:'flex-end', gap:1.2, mb:2 }}>
              <Box sx={{ width:30, height:30, borderRadius:'9px', bgcolor:`${GREEN}20`, border:`1px solid ${GREEN}40`, display:'flex', alignItems:'center', justifyContent:'center', color:GREEN, flexShrink:0 }}><TroubleshootIconSm /></Box>
          ...
          {active.messages.length > 1 && (
            <Box sx={{ display:'flex', gap:1, mb:1.5, overflowX:'auto', '&::-webkit-scrollbar':{ display:'none' } }}>
              {PROMPTS.slice(0,4).map(p => (
                <Box key={p} onClick={() => send(p)} sx={{ px:1.4, py:0.5, borderRadius:'16px', border:`1px solid ${BORDER}`, bgcolor:CARD, cursor:'pointer', whiteSpace:'nowrap', flexShrink:0, transition:'all 0.15s', '&:hover':{ border:`1px solid ${GREEN}44`, bgcolor:`${GREEN}08` } }}>
                  <Typography sx={{ fontSize:'0.7rem', color:'#666' }}>{p}</Typography>
                </Box>
              ))}
            </Box>
          )}
          <TextField fullWidth multiline maxRows={4} placeholder="Describe your network issue..." value={input}
            onChange={e => setInput(e.target.value)}
            onKeyDown={e => { if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();} }}
            sx={{ '& .MuiOutlinedInput-root':{ bgcolor:CARD2, borderRadius:'12px', color:'#ccc', fontSize:'0.85rem', '& textarea':{ '&::placeholder':{ color:'#444', opacity:1 } }, '& fieldset':{ border:`1px solid ${BORDER}` }, '&:hover fieldset':{ border:`1px solid #2a2a2a` }, '&.Mui-focused fieldset':{ border:`1px solid ${GREEN}55` } } }}
            InputProps={{ endAdornment:(
              <InputAdornment position="end">
                <IconButton onClick={() => send()} disabled={!input.trim()||loading} sx={{ width:34, height:34, bgcolor:input.trim()&&!loading?GREEN:'#1a1a1a', borderRadius:'9px', color:input.trim()&&!loading?'#000':'#444', transition:'all 0.2s', '&:hover':{ bgcolor:input.trim()&&!loading?'#2ecc9a':'#1a1a1a' }, '&.Mui-disabled':{ bgcolor:'#1a1a1a', color:'#333' } }}>
                  <SendIcon />
                </IconButton>
              </InputAdornment>
            )}}
      ...
      <ArchiveModal open={archiveModal} sessions={archivedSessions} onRestore={id => { handleRestore(id); }} onDelete={handleDelete} onClose={() => setArchiveModal(false)} />

      {sessionMenu && (
        <PopupMenu x={sessionMenu.x} y={sessionMenu.y} items={getSessionMenuItems(sessionMenu.sessionId)} onClose={() => setSessionMenu(null)} />
      )}
      {movePickerMenu && (
        <MoveToThreadPicker x={movePickerMenu.x + 175} y={movePickerMenu.y} threads={threads}
          onMove={threadId => handleMoveToThread(movePickerMenu.sessionId, threadId)}
          onClose={() => setMovePickerMenu(null)}
        />
      )}
    </Box>
  );
